...
class PathScript
	...
	def run
		...
		xi = 0
		yi = 0
		ti = Time.new(2015,03,3,7).to_i
		tf = Time.new(2015,03,4,7).to_i
		priority = 0

		p = ProgressBar.create(:total => (tf.to_i - ti.to_i)/5+1)

		(ti.to_i..tf.to_i).step(5) { |t|
			xi = [0, [width, xi + step_size/2.0 - rand(1000*step_size)/1000 ].min].max
			yi = [0, [width, yi + step_size/2.0 - rand(1000*step_size)/1000 ].min].max
			priority += 0.005
			if (priority >= 1.3) 
				priority = 0
			end
